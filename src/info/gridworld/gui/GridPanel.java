 ...
 * @author Robert Glen Martin - Sudoku changes							// RGM
 */
...
import java.awt.Stroke;													// RGM
import java.awt.BasicStroke;											// RGM
        ...
        PseudoInfiniteViewport.Pannable
{
    ...
    private Color backgroundColor1 = Color.WHITE;						// RGM
    private Color backgroundColor2 = Color.LIGHT_GRAY;					// RGM
    ...
    private Stroke thickStroke = new BasicStroke(3.0F);					// RGM
    ...
    public void paintComponent(Graphics g)
    {
        ...
        g2.setColor(backgroundColor1);									// RGM
        g2.fillRect(insets.left, insets.top, numCols * (cellSize + 1) + 1, numRows
                * (cellSize + 1) + 1);

        g2.setColor(backgroundColor2);									// RGM
        for (int r = 0; r < 9; r += 3)									// RGM
        	for (int c = 0; c < 9; c += 3)								// RGM
        		if ((r + c) % 2 == 1)									// RGM
        			g2.fillRect(insets.left + 1 + c * (cellSize + 1),	// RGM
        						insets.top + 1 + r * (cellSize + 1),	// RGM
        						3 * (cellSize + 1) + 1,					// RGM
        						3 * (cellSize + 1) + 1);				// RGM
    ...
    private void drawGridlines(Graphics2D g2)
    {
		...
		Stroke saveStroke = g2.getStroke();								// RGM
        g2.setStroke(thickStroke);										// RGM
        for (int y = miny; y <= maxy; y += 3 * (cellSize + 1))				// RGM
            // draw horizontal lines									// RGM
            g2.drawLine(minx, y, maxx, y);								// RGM
																		// RGM
        for (int x = minx; x <= maxx; x += 3 * (cellSize + 1))				// RGM
            // draw vertical lines										// RGM
            g2.drawLine(x, miny, x, maxy);								// RGM
        g2.setStroke(saveStroke);										// RGM
    }
