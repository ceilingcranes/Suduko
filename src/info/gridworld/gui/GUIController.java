 ...
 * @author Robert Glen Martin - Modified for Sudoku
 */
...
public class GUIController<T>
{
    ...
    private static final int MIN_DIFFICULTY = 1, MAX_DIFFICULTY = 4;
    private static final int INITIAL_DELAY = MIN_DELAY_MSECS;
    //        + (MAX_DELAY_MSECS - MIN_DELAY_MSECS) / 2;			RGM
    private static final int INITIAL_DIFFICULTY = 1;			 // RGM
    private static final String[] locationNames =				// RGM
    	{"Internet", "Files"};									// RGM
    private static final int[] locationValues =					// RGM
    	{10, 11};												// RGM


    private Timer timer;
    // private JButton stepButton, runButton, stopButton;	RGM
    private JButton newButton, solveButton, clearButton; // RGM
    private JComboBox locCombo; // RGM
    ...
    private String puzzleLocation;	// RGM

    private int difficulty;	// RGM
            ...
            DisplayMap displayMap, ResourceBundle res)
    {
        ...
        difficulty = INITIAL_DIFFICULTY;	// RGM
        makeControls();
    ...
    private void makeControls()
    {
        ...
        */
        ...
        // Dimension spacer = new Dimension(5, stepButton.getPreferredSize().height + 10); RGM
        Dimension spacer = new Dimension(5, newButton.getPreferredSize().height + 10); // RGM

        controlPanel.add(Box.createRigidArea(spacer));

        // controlPanel.add(stepButton); RGM
        controlPanel.add(newButton);	// RGM
        controlPanel.add(Box.createRigidArea(spacer));
        // controlPanel.add(runButton);	RGM
        controlPanel.add(solveButton);	// RGM
        controlPanel.add(Box.createRigidArea(spacer));
        // controlPanel.add(stopButton); RGM
        controlPanel.add(clearButton);	// RGM
        // runButton.setEnabled(false);	RGM
        // stepButton.setEnabled(false);	RGM
        // stopButton.setEnabled(false);	RGM
        newButton.setEnabled(true);		// RGM
        solveButton.setEnabled(true);	// RGM
        clearButton.setEnabled(true);	// RGM

        controlPanel.add(Box.createRigidArea(spacer));
        // controlPanel.add(new JLabel(resources.getString("slider.gui.slow"))); RGM
        controlPanel.add(new JLabel("easy"));	// RGM
        // JSlider speedSlider = new JSlider(MIN_DELAY_MSECS, MAX_DELAY_MSECS, RGM
        //        INITIAL_DELAY);											   RGM
        JSlider speedSlider = new JSlider(MIN_DIFFICULTY, MAX_DIFFICULTY,
                INITIAL_DIFFICULTY);
        // speedSlider.setInverted(true); RGM
        ...
        // controlPanel.add(new JLabel(resources.getString("slider.gui.fast"))); RGM
        controlPanel.add(new JLabel("evil"));			// RGM
        controlPanel.add(Box.createRigidArea(new Dimension(5, 0)));

        //Create the combo box, select the item at index 0.  RGM
        locCombo = new JComboBox(locationNames);
        locCombo.setSelectedIndex(0);
		puzzleLocation = locationNames[0];
		locCombo.setPreferredSize(new Dimension(
			locCombo.getPreferredSize().width,
			newButton.getPreferredSize().height));
        locCombo.setMaximumSize(locCombo.getPreferredSize());
		controlPanel.add(locCombo);
		controlPanel.add(Box.createRigidArea(new Dimension(5, 0)));
        ...
        */
        ...
        newButton.addActionListener(new ActionListener()
        {
            ...
            {
            	int puzzleLoc = 0;
            	if (puzzleLocation.equals(locationNames[0]))
            		puzzleLoc = locationValues[0];
            	else if (puzzleLocation.equals(locationNames[1]))
            		puzzleLoc = locationValues[1];

                parentFrame.getWorld().newPuzzle(
                	difficulty, puzzleLoc);
            }
        });
        ...
        speedSlider.addChangeListener(new ChangeListener()
        {
            public void stateChanged(ChangeEvent evt)
            {
                difficulty = (((JSlider) evt.getSource()).getValue());
            }
        });
        locCombo.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent e)
            {
        		puzzleLocation = (String)locCombo.getSelectedItem();
            }
        });
    }
